...
public class TetrisGame{
	...
	public static void nextTetrisBlock() {
		...
		//choose random block
		int randNum = (int)(Math.random()*3)+1;//random number between 1 and 7
		if(randNum == 1)
	...
	private static boolean isFullRow(int i)
	{
		...
		for(int r = 1; r < 11 ;r++)
		{			
			if(gr.get(new Location(i, r)) == null)
			{
				return false;
		...
		{	
			if(isFullRow(i))
			{
				...
				for(int b = 17; b>0; b--)
				{
					for(int c = 1; c < 11; c++)
					{
						if(gr.get(new Location(b,c)) != null){
							gr.get(new Location(b,c)).moveTo(new Location(b+1,c));
						}
					}
				}
				i++;
